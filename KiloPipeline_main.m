...
location =                  [3, 4];                     % posterior/Lateral
...
runKilo_flag =              0;                          % Run Kilosort
runQualityControl_flag =    0;                          % Run Bombcell for Quality Control
runPhy_flag =               0;                          % Run Phy after sorting
...
Folders.datFolder = [Folders.O_NHP_Folder, Folders.NHP_Folder, '\', sesName, '\dat_', region];
...
if(getDat_flag)
    ...
    mkdir(['dat_', region]);
    cd(['dat_', region])
    datFolder = [Folders.KiloFolder, sesName, '\', 'dat_', region, '\'];
    datFile = [Folders.datFolder, '\', fileName, '.dat'];
...
if(runPhy_flag)
    ...
    command = strcat("activate phy2 && phy template-gui  ", Folders.datFolder, "\params.py");
