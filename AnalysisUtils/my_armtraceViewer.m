function my_armtraceViewer(Z_ML, varargin)
...
if(~isfield(Z_ML, 'ArmRT_Corr'))
Z_ML.ArmRT_Corr = [];
Z_ML.ArmDir_All_RL = [];

for i = 1:length(Z_ML.condition)

    if(Z_ML.TrialError(i) == 1)
        if(Z_ML.condition(i) == 5)
            diode_onset = 500;
        else
            diode_onset = 200;
        end
    
        if(strcmp(Z_ML.Target_Location(2, i),'Right'))
            df = [0, diff(Z_ML.TouchX_Diode{1, i})'];
            dfx = find(df(diode_onset:diode_onset+Z_ML.ArmRT(i)-1) < -1);
            if(~isempty(dfx))
                RL = 2;
            else
                RL = 1;
            ...
            dfx = find(df(diode_onset:diode_onset+Z_ML.ArmRT(i)-1) > 0.2);
            if(~isempty(dfx))
                RT = dfx(1) - 2;
            else
                RT = Z_ML.ArmRT(i);
    ...
    Z_ML.ArmRT_Corr(i) = RT;
    Z_ML.ArmDir_All_RL(i) = RL;
    end
...
% Create a text box
hTextBox2 = uicontrol('Style', 'edit', 'Position', [1200 20 80 40], 'String', 'RT');

% Create a button to save the text box input
hButton = uicontrol('Style', 'pushbutton', 'Position', [120 20 80 40], 'String', 'Save RT', ...
...
myData.Z_ML = Z_ML;
myData.params.tr = find(Z_ML.TrialError == 1 & Z_ML.condition == 5, 1);
...
subplot(1, 3, 1)
cla
plot(Z_ML.TouchX_Diode{tr}, 'b')
hold on
xline(Z_ML.ArmRT_Corr(tr) + diode_onset, 'k')
view([90 -90]);

f_arr = find(Z_ML.TrialError == 1 & Z_ML.condition == 5);

sgtitle(['Trial number:  ', num2str(tr), '     --      ', num2str(find(f_arr == tr, 1)), '  /  ', num2str(length(f_arr)),'     b: X,    r: Y'])

subplot(1, 3, 2)
cla
plot(Z_ML.TouchY_Diode{tr}, 'b')
hold on
xline(Z_ML.ArmRT_Corr(tr) + diode_onset, 'k')

subplot(1, 3, 3)
cla
plot(Z_ML.TouchX_Diode{tr}, Z_ML.TouchY_Diode{tr}, 'b')

if(Z_ML.ArmDir_All_RL(tr) == 1)
    set(myData.hTextBox, 'String', 'Right');
elseif(Z_ML.ArmDir_All_RL(tr) == 2)
    set(myData.hTextBox, 'String', 'Left');
elseif(Z_ML.ArmDir_All_RL(tr) == 3)
    set(myData.hTextBox, 'String', 'Fix');
else
    set(myData.hTextBox, 'String', 'no saccade');
end

set(myData.hTextBox2, 'String', num2str(Z_ML.ArmRT_Corr(tr)));
...
switch keydata.Key
    case 'rightarrow' % increment cluster index
        ...
        idx = find(Z_ML.TrialError == 1 & Z_ML.condition == 5);
...
%         end
    ...
    case 'r'
        Z_ML.ArmDir_All_RL(p.tr) = 1;

    case 'l'
        Z_ML.ArmDir_All_RL(p.tr) = 2;
...
%         Z_ML.EyeDir_All_UD(p.tr, p.sac_idx) = 2;
    ...
    case 'f'
        ...
        Z_ML.ArmRT_Corr(p.tr) = str2num(get(myData.hTextBox2, 'String'));
...
p = myData.params;
myData.textBoxData2 = get(myData.hTextBox2, 'String');
myData.Z_ML.ArmRT_Corr(p.tr) = str2num(get(myData.hTextBox2, 'String'));
set(f, 'UserData', myData);
end