...
nUnits  = numel(su); 
nUnits = length(find([su.clusterScore] == 2));
...
newarray = find([su.clusterScore] == 2);

supeak = [su.peakCh];
[~, idxSort] = sort(supeak(newarray));

%%
iUU = 0;
iPlot = 1;
for iU = newarray(idxSort)
    thisUnit = su(iU);
    ...
    if isfield(thisUnit, 'medWfOnPeakCh')
        plot(linspace(-0.5, 1.25, length(-round(0.5/1000*30000):round(1.25/1000*30000))), thisUnit.medWfOnPeakCh, 'LineWidth', 2, 'Color', clr(iUU,:))
    end
%     ylim([-5e4 5e4])
    ylabel(['Cell ' num2str(thisUnit.clusterId)], 'Color', clr(iUU,:))
    ...
    isi(isi > isiDistMax) = [];
    histogram(isi, 50, 'FaceColor', clr(iUU,:));
    xlim([0 isiDistMax])
    ...
    plot(spikeCounts, 'Color', clr(iUU,:), 'LineWidth', 2)
    ...
    stairs([-x(end:-1:1) x]-binSize/2,[n(end:-1:1) n], 'Color', clr(iUU,:), 'LineWidth', 2.0);
    yl = ylim(); 
    ylim([0 yl(2)]);
    ...
    if isfield(thisUnit, 'isiV_rate')
        text(0.1, yPosStart, ['isiV = ', sprintf('%.5f', thisUnit.isiV_rate)]);
        yPosStart = yPosStart+0.2;
    end
    if isfield(thisUnit, 'isiV_fpRate')
        text(0.1, yPosStart, ['fpRate = ', sprintf('%.5f', thisUnit.isiV_fpRate)]);
        yPosStart = yPosStart+0.2;
    end
