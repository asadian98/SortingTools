function [sp, wf, wfOnPeak] = getSp(ksDir, datDir, sesName, datpath, n_channels, varargin)
...
sp.dat_path = datpath;
sp.n_channels_dat = n_channels;
sp.dtype = 'int16';
sp.offset = 0;
sp.sample_rate = 30000;
...
%     gain = 0.6/512/500*1e6; % raw file units to uV ***SHOULD BE RIG SPECIFIC. NEED TO DO THIS...
    ...
    % median wf per cluster per channel, size: [nClu, nCh, nSamps]
        ...
        sp.medWfs = extractMedianWFs(sp.spikeClusters, sp.spikeTimesSecs, sp.sample_rate, fullfile(datDir, sp.dat_path), sp.dtype, dataSize, chanMap, gain);
...
%     load(fullfile(ksDir, 'ops.mat'));
    ...
    Fs = sp.sample_rate;
    datPath = fullfile(datDir, sp.dat_path);
    dataType = sp.dtype;
    d = dir([datDir '/*.dat']);
    nSamp = d.bytes/2/sp.n_channels_dat;
    dataSize = [sp.n_channels_dat nSamp];
