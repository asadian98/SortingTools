function [psth, bins, rasterX, rasterY, spikeCounts, binnedArray] = psthAndBA(spikeTimes, eventTimes, window, psthBinSize, kernel)
...
[binnedArray, bins] = timestampsToBinned(spikeTimes, eventTimes, psthBinSize, window);
 ...
 % Convolve with the EPSP kernel
convolvedArray = conv2(binnedArray, kernel(:,2)', 'same');

spikeCounts = sum(convolvedArray,2);
psth = mean(convolvedArray./psthBinSize); % normalize to Hz

[tr,b] = find(convolvedArray);
